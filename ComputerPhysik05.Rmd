...
plot(1:200, schritte, "l", ylim=c(0, 0.05))
```
...
Mit dem Ziel die Schwingung in ihre Frequenzen aufzuteilen, werden wir die Werte der zeitlichen Entwicklung Fourier transformieren. Diese Transformation zerlegt eine reele Funktion in eine Summe von gewichteten Cosinusfunktionen. Wir werden den Fast Fourier Transform-Algorithmus verwenden, da wir mit diskreten Werten arbeiten. Die $n$ erhaltenen Werte entsprechen anschließend den Gewichtungen der Cosini in aufsteigender Wertigkeit, also:
...
for (x in 1:5){
  ...
  lines(1:2^12, ff1,"l", col = rainbow(5)[x])
}
```
